...
var moeda = {
	 ...
	 * @param  $ {Object} num
	 */
	formatar: function(num,casasDecimais)
	{
        if(!casasDecimais){
            var casasDecimais=2;
        }	
        ...
        cents = Number(num.toString().split('.')[1]);//Math . floor((num * 100 + 0.5) % 100);
		num = num.toString().split('.')[0];//Math . floor((num * 100 + 0.5) / 100) . toString();
		tamanhoCents = (cents.toString()).length;
        if(casasDecimais > tamanhoCents){
            falta = casasDecimais - tamanhoCents;
            for(i=0;i<falta;i++){
                cents = "0" + cents;
            }
        }
