...
--	 set 	@nTeste = 1
	...
	INSERT INTO OnPlaceSANED_movimento..ACQ_MATRICULA
		(
			seq_matricula,
			seq_matricula_principal, 
			seq_pessoa_proprietario, 
			cod_uf, 
			seq_localidade, 
			seq_rota, 
			seq_leitura, 
			des_inscricao_imobiliaria, 
			des_registro_imovel, 
			seq_setor, 
			seq_logradouro, 
			val_numero_imovel, 
	...
	select	
			cg_matricula, 
			case when cg_matricula_pai = 0 then null else cg_matricula_pai end, 
			cg_matricula, 
			'SP', 
			1,
			cg_grupo, 
			cg_sequencia, 
			OnPlaceSaned.dbo.FC_COMPLETA_ZEROS(cg_setor, 3) + cg_inscricao, 
			'', 
			cg_setor, 
			dbo.fc_busca_logradouro(ltrim(rtrim(cg_endereco))), 
			cg_num_imovel, 
			cg_complemento, 
