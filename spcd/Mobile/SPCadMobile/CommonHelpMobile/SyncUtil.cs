...
{
    /// <summary>
    /// Converte o Objeto para classe esperada
    /// </summary>
    /// <typeparam name="T">Classe nativa</typeparam>
    /// <typeparam name="T2">Objeto externo</typeparam>
                ...
                foreach (PropertyInfo prop in typeof(T2).GetProperties())
                {
                    ...
                    if (property != null)
                    {
                        ...
                        {                            
                            if (!tipoNulo)
                            {
                                object valor = (prop.GetValue(obj, null) == null) ? null : prop.GetValue(obj, null);
                                property.SetValue(retorno, bool.Parse(valor.ToString()), null);
                            }
                            else
                            {
                                property.SetValue(retorno, null, null);
            ...
            foreach (T obj in lstEntrada)
            {
                ...
                foreach (PropertyInfo prop in typeof(T).GetProperties())
                {
                    ...
                    {                        
                        ...
                        else if (prop.GetValue(obj, null).ToString() == "")
                        {
                            tipoNulo = true;
                            string nullo = NullableType(property);
                            //property.SetValue(retorno, ConvertString(nullo, ""), null);
